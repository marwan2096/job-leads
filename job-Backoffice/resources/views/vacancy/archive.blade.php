...
<x-alert />
    ...
    <a href="{{ route('vacancy.show', $vacancy) }}" class="text-slate-600 hover:text-slate-900 hover:underline uppercase">
        {{ $vacancy->title }}
                <span class="text-xs text-gray-400 group-hover:text-gray-600">→ view details</span>
                      ...
                      <td class="px-6 py-4">{{ Str::limit($vacancy->description, 50) }}</td>
                        ...
                        <td class="px-6 py-4">{{ $vacancy->Company->name ?? 'N/A' }}</td>

                        <td class="px-6 py-4 flex space-x-3">
                           <form action="{{ route('vacancy.forceDelete', $vacancy) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 hover:text-red-700">🗃️ delete</button>
                            </form>

                            <form action="{{ route('vacancy.restore', $vacancy) }}" method="POST" class="inline-block">
                                @csrf
                                @method('PUT')
        ...
        <a href="{{ route('vacancy.index') }}" class="text-blue-500 hover:text-blue-700 flex justify-end mt-3 mr-4">
            ← Back to Vacancies
        </a>
